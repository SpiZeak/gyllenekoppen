+++
title = "Hur man brygger pour-over"
description = "En lugn och medveten bryggmetod som belönar tålamod med en kristallklar, aromrik kopp. Perfekt för ljusröstade single origin-bönor."
template = "guide_page.html"
weight = 2

[extra]
ikon = "🫖"
image = "photo-1461023058943-07fcbe16d735"
svarighet = "Nybörjarvänlig"
tidsatgang = "ca 10 min"
dryck = "Pour-Over"
malningsgrad = "Mediumfin (som bordssalt)"
temperatur = "93–95°C"
vattenkvot = "1:15 (20g → 300g)"

[[extra.utrustning]]
namn = "Pour-over dripper (t.ex. Hario V60)"
ikon = "🫖"

[[extra.utrustning]]
namn = "Pappersfilter"
ikon = "📄"

[[extra.utrustning]]
namn = "Gooseneck-kanna"
ikon = "🫗"

[[extra.utrustning]]
namn = "Burr-kvarn"
ikon = "🔧"

[[extra.utrustning]]
namn = "Våg med timer"
ikon = "⚖️"

[[extra.utrustning]]
namn = "Termometer"
ikon = "🌡️"

[[extra.tips]]
titel = "Håll hällningen konsekvent"
text = "Undvik att hälla för snabbt – långsam, kontrollerad hällning ger jämn extraktion och bättre smak."

[[extra.tips]]
titel = "Bloom-effekten avslöjar färskheten"
text = "En kraftig bloom betyder fräscha bönor. Liten eller ingen bloom = gamla bönor."

[[extra.tips]]
titel = "Experimentera med förhållanden"
text = "Prova 1:14 för en kraftigare kopp eller 1:16 för en mildare. Justera efter din smak."
+++

## Vad du behöver

- Pour-over-dripper (t.ex. Hario V60, Chemex eller Kalita Wave)
- Pappersfilter som passar din dripper
- Gooseneck-kanna (pip-kanna för precision) eller vanlig kanna
- Kvarn (burr-kvarn rekommenderas)
- Våg med timer
- Termometer eller smart vattenkokare
- Mugg eller serverkanna

---

## Steg 1 &ndash; Värm upp och spöla filtret

Kok vatten och låt det svalna till ca 93&ndash;95&deg;C. Placera filtret i drippern och spöla det med hett vatten &ndash; detta tar bort papperssmaken och värmer upp utrustningen.

Häll bort spölvattnet från muggen.

---

## Steg 2 &ndash; Väg upp och mala bönorna

**Recept för en kopp (300&thinsp;ml):**

- Kaffe: 20&thinsp;g
- Vatten: 300&thinsp;g (1:15-förhållande)

Mala kaffet **mediumfint** &ndash; ungefär som bordssalt. Lägg kaffet i det spolade filtret och skaka drippern försiktigt för att jämna ut ytan.

---

## Steg 3 &ndash; Bloom (preskvättning)

Placera drippern på vågen och ställ om den till noll. Starta timern.

Häll 40&thinsp;g vatten (dubbelt mot kaffemängden) jämnt över kaffet. Se till att allt kaffe blöts.

Vänta **30&ndash;45 sekunder**. Du ser kaffet "bloomma" &ndash; det bubblar och sväller upp. Det är CO&sup2; som släpps ut från fräscha bönor.

---

## Steg 4 &ndash; Första hällning

Vid 45 sekunder, börja hälla vatten i jämna, cirkulära rörelser &ndash; börja från mitten och spirala utåt mot kanten, sedan tillbaka.

Häll sakta och kontrollerat. Mål: nå **150&thinsp;g** vid ca 1:15.

---

## Steg 5 &ndash; Andra och tredje hällning

Fortsätt hälla i omgångar när vattennivån sjunker, men för inte ned den hela vägen före nästa hällning.

- **Vid 1:30** &ndash; häll till 220&thinsp;g
- **Vid 2:00** &ndash; häll till 300&thinsp;g (totalvikten)

---

## Steg 6 &ndash; Dränering

Kaffet borde vara färdigt dränerat vid **3:00&ndash;3:30**. Om det tar längre tid är malningen för fin; om det går snabbare är den för grov.

När dräneringen är klar, ta bort drippern och njut.

---

## Steg 7 &ndash; Smaktest och justering

| Smak         | Orsak            | Åtgärd                      |
| ------------ | ---------------- | --------------------------- |
| Surt, tunt   | Underextraherat  | Finare malning              |
| Beskt, torrt | Överextraherat   | Grovare malning             |
| Svagt        | För lite kaffe   | Öka dos till 22–25&thinsp;g |
| För stark    | För mycket kaffe | Minska dos                  |

---

## Tips för bättre pour-over

- **Håll hällningen konsekvent** &ndash; undvik att hälla för snabbt; långsam, kontrollerad hällning ger jämn extraktion.
- **Använd fräscha bönor** &ndash; bloom-effekten påvisar fräschheten. Liten bloom = gamla bönor.
- **Experimentera med förhållanden** &ndash; prova 1:14 för kraftigare, 1:16 för mildare kopp.
- **Kallt brygg** &rarr; använd isvatten för cold brew istället.
